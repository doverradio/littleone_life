...
const PieChartMysteries = ({ data }) => {
  ...
  const drawChart = () => {
    ...
    const colorScale = d3.scaleOrdinal()
        .domain(Object.keys(mysteryColors)) // Use the keys of the mysteryColors object
        .range(Object.values(mysteryColors)); // Use the values of the mysteryColors object
    ...
    legendItems
        .on('mouseover', d => highlightSlice(d.label, true))
        .on('mouseout', d => highlightSlice(d.label, false))
        .on('click', d => highlightSlice(d.label, true));
